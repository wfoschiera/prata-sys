...
- The PR must include exactly one label from the required set: `feat`, `fix`, `chore`, `refact`, `docs`, `upgrade`, `breaking`, `security`, `bug`, `feature`, `internal`, `lang-all`. The `check-labels` CI job will fail without it.
