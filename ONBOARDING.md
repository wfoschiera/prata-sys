...
>
> This guide is grounded in the code as of `main` (`a8b4a4d`, 2026-07-16). Every factual
...
- **Auth & RBAC:** JWT (HS256) + a two-tier permission model — role defaults ∪ per-user DB overrides,
  cached per request (`backend/app/api/deps.py:68-95`). Roles: `admin`, `finance`, `client`.
...
- **Where to start reading:** `backend/app/models.py`, `backend/app/crud.py` (stock logic
  `crud.py:381-510`), `backend/app/api/deps.py` (auth/RBAC), `frontend/src/main.tsx` (app wiring),
...
- **Backend** mounts every router under `settings.API_V1_STR` = `/api/v1` (`backend/app/core/config.py:32`,
...
2. **Auth + RBAC** — FastAPI resolves the guard before the handler body.
   `require_permission("manage_services")` (`backend/app/api/deps.py:68-95`) depends on `CurrentUser`
...
3. **Body validation** — Pydantic validates the `ServiceCreate` body (`models.py:396-399`:
...
4. **Handler** — looks up the client via `session.get(Client, ...)`, 404s if absent, then calls
   `crud.create_service(...)` (`services.py:51-54`).
5. **CRUD** — `create_service` (`backend/app/crud.py:291-331`): `Service.model_validate` → `add` →
   `commit` → `refresh`, then re-fetches via `get_service` (`crud.py:253-266`) which eager-loads
   `client`, `items`, `status_logs` with `selectinload` to avoid N+1 at serialization.
6. **Model/DB** — inserts into table `service` (`models.py:453-482`): `status` defaults to
   `ServiceStatus.requested` (`models.py:331`, `348-354`), `created_at` via `get_datetime_utc()`. No
...
7. **Response** — the ORM object is serialized through `ServiceRead` (`models.py:417-432`) and returned
   as HTTP 201.
...
|---|---|---|
| `User` | `user` | `user_permissions` → `UserPermission` (cascade delete). SQLModel default `client` (`models.py:28`); DB column shipped with `server_default='admin'` for migration safety. |
| `UserPermission` | `user_permission` | FK `user_id` (CASCADE), unique `(user_id, permission)` (`models.py:76-86`). |
| `Client` | `client` | unique `document_number`; structured address (`bairro`, `city`, `state`, `cep`) (`models.py:250-269`). |
| `Service` | `service` | FK `client_id`; `items` → `ServiceItem` (cascade), `status_logs` → `ServiceStatusLog`, `product_items` → `ProductItem` (`models.py:453-482`). |
...
| `ProductType` → `Product` → `ProductItem` | `producttype`/`product`/`productitem` | `Product` FK `product_type_id` (RESTRICT) + optional `fornecedor_id`; `ProductItem` is the physical stock unit (`models.py:870-952`). |
| `Fornecedor` | `fornecedor` | `contatos`, `categorias` (both cascade), `transacoes` (`models.py:809-861`). |
| `Transacao` | `transacao` | nullable FKs `service_id`/`client_id`/`fornecedor_id` (SET NULL); `valor` `Numeric(12,2)` (`models.py:701-734`). |
| `Orcamento` | `orcamento` | unique `ref_code`, FK `client_id`, FK `service_id` (set on conversion), `items`/`status_logs`, `created_by_user` (`models.py:1225-1267`). |
| `CompanySettings` | `company_settings` | singleton letterhead for orçamento documents (`models.py:114-128`). |

### Enums & status lifecycles

- **Service** — `ServiceStatus`: `requested → scheduled → executing → completed`, and any non-terminal
  → `cancelled`. Enforced by `VALID_STATUS_TRANSITIONS` (`models.py:345-352`, verified).
- **Stock** — `ProductItemStatus`: `em_estoque → reservado → utilizado` (`models.py:864-869`).
  Forward transitions are validated in `crud.validate_product_item_transition`
  (`crud.py:1147+`); the cancel-release path `reservado → em_estoque` bypasses that helper and is
  handled directly in `_release_stock_items` (`crud.py:366-380`).
- **Orçamento** — `OrcamentoStatus`: `rascunho ⇄ em_analise`, `em_analise → aprovado`,
  `aprovado ⇄ em_analise`, any non-terminal → `cancelado` (`models.py:1088-1098`).
...
- **Stock reservation is best-effort/FIFO** (see §5) — a shortfall produces a `StockWarning` but does
  **not** block the transition (`crud.py:419-431`).
- **Transaction category must match tipo**, and `client_id` is only allowed on a `receita`; enforced by
  ...
  (`models.py:570-586`, `630-651`). `tipo` is immutable on update (omitted from `TransacaoUpdate`,
  `models.py:660-661`).
- **CNPJ is validated with the July-2026 alphanumeric rule** (ASCII-48 + mod-11 check digits),
  CPF must be 11 digits (`models.py:262-296`).
- **Password reset tokens are single-use** within the expiry window, tracked in
  `used_password_reset_token` (`models.py:149-159`).

The migration history (`backend/app/alembic/versions/`, 22 files, single linear chain) shows the
...
`ROLE_PERMISSIONS` defaults (`permissions.py:48-78`) unioned with per-user `UserPermission` overrides,
...
(`deps.py:68-95`, verified), which **short-circuits for superusers** and caches the effective set on
`request.state._cached_permissions` so multiple guards on one endpoint hit the DB once. `crud.set_user_permissions`
only persists true overrides (strips role-default redundancy, `crud.py:168-171`). *Gotcha:* `admin`
...
**Service lifecycle + stock.** `transition_service_status` (`crud.py:511-573`) validates against
...
`_check_stock_for_service` (`crud.py:381-432`, verified) is the concurrency-critical path: for each
material `ServiceItem` it runs `SELECT ... FOR UPDATE` (`with_for_update()`, `crud.py:406`) ordered by
...
(legacy). `_deduct_stock_items` (`crud.py:433-510`, verified) honors the per-item quantities in
`deduction_items`: it consumes reserved items **FIFO up to each requested amount**, then releases any
leftover reservations back to `em_estoque` when the service reaches a terminal state.

**Quotes (orçamentos).** Full document lifecycle with a unique 6-char `ref_code`
(`_generate_ref_code`, `crud.py:1349`). `transition_orcamento_status` (`crud.py:1473+`) guards
...
(`crud.py:1668+`) runs three grouped aggregation queries by ISO week (completed-service counts by
...
**First-time setup** (`scripts/dev-setup.sh:1-65` — idempotent): validates `.env`/`compose.dev.yml`/
...
1. **test** — spins up `postgres:18`, installs Python 3.14 + uv, migrates via `prestart.sh`, runs
   `scripts/test.sh` (`deploy.yml:21-68`).
2. **build** — builds backend (`backend/Dockerfile`) and frontend (`frontend/Dockerfile`) images,
   pushes to GHCR tagged both `:latest` and `:sha-<7>` (`deploy.yml:69-119`). Frontend is built with
...
3. **deploy** — joins the private mesh via Tailscale, `scp`s only `compose.prod.yml` + `Caddyfile` to
   ...
   and `... up -d --wait --remove-orphans` (`deploy.yml:120-174`). Passing `TAG` through `sudo env`
   is what pins the sha — otherwise sudo resets the environment and compose falls back to
   `${TAG:-latest}` (commit `d5c3533`).

**Migrations on deploy:** run at container start — `compose.prod.yml:27` executes
...
- The host holds a hand-written `.env` (gitignored since commit `6423615`); required prod vars have no
  ...
  runbook in `deploy/README.md`; `changethis` secrets are **rejected** in prod (`config.py:107-116`).
...
- **Disciplined N+1 avoidance.** `selectinload` is used consistently for related objects, and read
  ...
  `status_logs` but `get_services` omits them (`crud.py:277-281`), backed by `ServiceRead` vs
  `ServiceListRead` (`models.py:417-447`). For a small team this prevents the most common perf
  regression without ORM complexity.
- **Concurrency-safe stock reservation.** Using `SELECT ... FOR UPDATE` with FIFO ordering
  (`crud.py:400-408`) is the correct primitive to prevent double-reservation of a physical stock unit
...
- **Sensible RBAC for the size.** Role defaults + sparse per-user overrides + per-request caching
  (`deps.py:86-90`) gives flexibility without an external policy engine, and the per-request cache
...
- ~~**No HTTP readiness endpoint (correctness/ops).**~~ ✅ Resolved: `GET /api/v1/utils/readiness/`
  now runs a `SELECT 1` DB probe (`routes/utils.py:39-40`), alongside the liveness-only
  `GET /api/v1/utils/health-check/` (`routes/utils.py:34-37`). Both paths are covered by tests.
- ~~**`_deduct_stock_items` ignores per-item quantities (correctness).**~~ ✅ Resolved: the function
  now consumes reserved items FIFO honoring the `deduction_items` payload and releases leftovers
  back to `em_estoque` (`crud.py:433-510`).
...
|---|---|---|---|---|---|
| 1 | ~~Stock deduction ignores per-item quantities~~ ✅ resolved | correctness/bug | High | `backend/app/crud.py:446+` | `_deduct_stock_items` now honors per-item quantities FIFO and releases leftover reservations. |
| 2 | ~~Health-check does no DB probe; no readiness endpoint~~ ✅ resolved | correctness | High | `backend/app/api/routes/utils.py:34-40` | `GET /utils/readiness/` runs `SELECT 1` (with tests for ok/db-down paths). |
...
| 7 | No concurrency test for FOR UPDATE reservation | test-gap | Med | `backend/app/crud.py:381-432`, `backend/tests/crud/` | Add a two-connection test that asserts no double-reservation. |
...
| 16 | Backend Dockerfile 4 workers vs prod 2 | docs | Low | `backend/Dockerfile:42`, `compose.prod.yml:27` | Align or add a comment explaining the override. |

### Paste-ready GitHub issues checklist

- [ ] Stock deduction ignores per-item quantities in `_deduct_stock_items` (`crud.py:433-510`) `severity:high` `correctness`
...
- [ ] Add a concurrency test for the FOR UPDATE stock reservation race (`crud.py:381-432`) `severity:med` `test-gap`
...
- [ ] Read these key files, in order: `backend/app/models.py` (enums + transition maps),
      `backend/app/crud.py:381-510` (stock + transitions), `backend/app/api/deps.py:68-95` (RBAC),
      `frontend/src/main.tsx` (wiring), `frontend/src/routes/_layout.tsx` (auth guard).
- [ ] Skim the knowledge graph: open `graphify-out/graph.html`, browse `graphify-out/wiki/index.md`,
      and try `/graphify query "how does stock reservation work?"`.
...
*Guide reflects the code at `main` (`a8b4a4d`, 2026-07-16). When code moves, cited line
