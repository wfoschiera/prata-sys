...
### Knowledge graph
`graphify-out/` holds a queryable knowledge graph of this repo (graph.json + wiki + HTML).
For architecture/"how does X work" questions, prefer `/graphify query "..."` before grepping.
Rebuild after big refactors with `/graphify . --wiki --update`.
