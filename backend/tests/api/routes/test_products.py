"""Per-router tests for the products endpoints.
...
update/delete and the ``/{id}/items`` sub-route, plus the permission matrix
(client-role forbidden, unauthenticated).
"""
...
def test_product_items_subroute_not_found(
    client: TestClient, admin_token_headers: dict[str, str]
) -> None:
    resp = client.get(f"{PREFIX}/{uuid.uuid4()}/items", headers=admin_token_headers)
    assert resp.status_code == HTTPStatus.NOT_FOUND


def test_list_products_client_forbidden(
...
def test_list_products_unauthenticated(client: TestClient) -> None:
